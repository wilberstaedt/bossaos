...
ETAPAS_FEITAS=$(grep -cE '^\| E[0-9]{2} \| (validado|implementado aguardando validação)' docs/progress/ETAPAS.md 2>/dev/null || true)
...
echo "PCT_ETAPA=$PCT_ETAPA ETAPAS=$ETAPAS_FEITAS/$ETAPAS_TOTAL PCT_TELA=$PCT_TELA TELAS=$TELAS_FEITAS/$TELAS_TOTAL ATUAL=$ATUAL"
