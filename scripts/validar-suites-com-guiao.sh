...
ns2-capturas.spec.ts:corre dentro da scripts/validar-sistema-ns2.sh, no projecto chromium. Produz as quatro capturas obrigatorias da landing do §9 e recusa-se a capturar se a pagina nao tiver os 7 blocos do norte - uma captura da interpretacao rejeitada sairia igualmente nitida. Mesma razao das anteriores: o corredor e uma guarda validar-*, que a descoberta daqui nao varre
ns2-mesas.spec.ts:corre dentro da scripts/validar-sistema-ns2.sh, no projecto painel porque a sala e autenticada. Produz as duas capturas das Mesas e mede se a primeira mesa cai dentro da primeira viewport, com controlo negativo proprio. Mesma razao das anteriores: o corredor e uma guarda validar-*, que a descoberta daqui nao varre
ns2-idiomas.spec.ts:corre dentro da scripts/validar-sistema-ns2.sh, no projecto painel. Mede as duas referencias do norte em es-ES, pt-BR e en, mais zoom a 200%, conteudo longo, foco e teclado - a cobertura que existia no repositorio mas cuja populacao nao continha o /floor. Mesma razao das anteriores: o corredor e uma guarda validar-*, que a descoberta daqui nao varre
