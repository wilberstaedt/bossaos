#!/usr/bin/env bash
#
# Os METADADOS, hreflang e partilha (§6.8), medidos antes e depois.
#
...
#
#   ./scripts/provar-seo-mkt.sh antes | depois | (nada)
#
...
FASE="${1:-}"
DESTINO="docs/visual/rv100/2026-09-06_e953a87/evidence/seo"
naomedi() { printf '  NAO MEDI %s\n' "$1"; exit 2; }

case "$FASE" in ''|antes|depois) ;; *) naomedi "fase '$FASE' desconhecida" ;; esac
[ -f inspeccao/rv100-seo.spec.ts ] || naomedi "o instrumento nao esta em inspeccao/"
...
echo "1. O instrumento dos metadados corre${FASE:+ (fase: $FASE)}"
RV100_FASE="$FASE" PORTA_INSPECCAO="$PORTA" NEXT_DIST_DIR="${NEXT_DIST_DIR:-.next}" \
  npx playwright test inspeccao/rv100-seo.spec.ts --project=chromium --workers=1
saida=$?
[ "$saida" -eq 0 ] || { printf '  FALHA o instrumento reprovou (saida %s)\n' "$saida"; exit 1; }

if [ -n "$FASE" ]; then
  echo; echo "2. E a evidencia ficou escrita"
  [ -s "$DESTINO/$FASE-seo.json" ] \
    && printf '  ok    %s\n' "$DESTINO/$FASE-seo.json" \
    || { printf '  FALHA %s nao foi escrito\n' "$DESTINO/$FASE-seo.json"; exit 1; }
fi
echo; echo "OK"
