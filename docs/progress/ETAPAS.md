...
| E08 | implementado, aguardando validação | docs/progress/E08.md · migração `e08_media_traducoes_importacao_publicacao` (8 tabelas; `menu_revisions` sem UPDATE nem DELETE para o runtime) · **publicar é atómico por construção** — a revisão e a troca do ponteiro no mesmo COMMIT · CSV neutralizado, SVG recusado pelos bytes, três portas contra a rede interna · nome igual nunca funde produtos · exportação verificada duas vezes · 221 asserções no domínio + 32 contra a base, **33 controlos negativos** · 10 telas |
| E09 | planejado |  |
| E10 | planejado |  |
| E11 | planejado |  |
| E12 | planejado |  |
