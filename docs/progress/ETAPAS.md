...
| E13 | implementado aguardando validação | docs/progress/E13.md · migração `e13_sala_sessoes_dispositivos` (11 tabelas, **dois índices únicos PARCIAIS**, `table_session_events` append-only por privilégio, `table_sessions` sem DELETE) · `provar-sala.sh` a 0 com **3 grupos, 22 casos e SETE controlos negativos** · a concorrência prova-se com **duas transacções demonstravelmente abertas ao mesmo tempo**, com o bloqueio da segunda MEDIDO antes do COMMIT da primeira — e não com `Promise.all` só · **17 telas** (a régua diz 16; a matriz tem 17, enumeradas no spec) com móvel medido · a regra **3-bis** do `offline-e-fila-local` fica DECIDIDA: a revogação descarta e diz-se ao revogar · `validar-concorrencia.sh` estava **cega em dois sítios** e saía a verde — corrigida, com autoteste, e a morder em três degradações |
| E14 | planejado |  |
| E15 | planejado |  |
| E16 | planejado |  |
| E17 | planejado |  |
