...
| E12 | implementado aguardando validação | docs/progress/E12.md · 4 migrações (`publico_tema`, `restaura_de_id`, rascunho+registo+imutabilidade, `agendar_descida`) · `provar-tema.sh` a 0 com **7 grupos, 29 casos e 6 controlos negativos** · `provar-tema-no-navegador.sh` lê a cor **calculada** por `getComputedStyle` na carta e no site públicos, antes e depois de publicar pelo produto, com 2 controlos a derrubar a leitura · **18 casos de navegador do tema** (20 no relatório, com as 2 sessões do arnês) · **7 telas com móvel medido** no inquilino Pro · dois defeitos reais encontrados fora da etapa: descida agendada à meia-noite **UTC** (caía no dia anterior a oeste de Greenwich) e `PUT /tema` a devolver **500** a uma cor que não é cor |
| E13 | planejado |  |
| E14 | planejado |  |
| E15 | planejado |  |
| E16 | planejado |  |
