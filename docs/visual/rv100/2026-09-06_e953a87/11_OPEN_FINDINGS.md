...
evidence: schema.prisma (colunas `@db.Uuid` em 54, 234, 235) · detector das 40 páginas com controlo positivo
evidence_retirada: as capturas M03-erro e M04-erro «em branco a 5851 e 2740 bytes» foram RETIRADAS desta linha a 07/09. O branco era a velocidade de obturador do arnês (`domcontentloaded` a fotografar antes de a interface chegar no fluxo), não a falha. O finding MANTÉM-SE — foi confirmado por duas provas independentes do arnês — mas quem o releia não deve aprender que um ecrã branco indica um 500, porque não indica
...
decisao: nao-precisa-de-autorizacao
status: corrigido NA CLASSE — VERIFICADO PELO REVISOR. O mecanismo vive em `packages/db/src/escopo.ts` e traduz o `P2023` do Prisma (que vem do `22P02` do Postgres) num 404, dos dois lados: o nome na base e o `notFound()` na web. A guarda `validar-id-de-rota-validado.sh` corre a zero e **a partição fecha**: 128 páginas sob um segmento de id, 128 alcançam o invólucro, **0 vão à base por fora dele e 0 não tocam na base** — completude e não piso, que foi a lição que o próprio JR tirou do seu `> 0`. Sonda a acender numa página que escapa. A contagem reconcilia com a minha: 164 ficheiros = 128 páginas + 33 de `api/` (que devolvem JSON e validam à mão) + layouts. FECHADO A 07/09 ÀS 11h35, COM PROVA MINHA NOS DOIS LADOS. Percurso: a primeira cura estava INERTE (escutava `P2023` e o erro era `P2007`), e a minha guarda dava 128/128 por cima dela; a segunda media o HTTP mas o controlo negativo só cobria a via de sessão. Medi e reportei; ele encontrou o resto: os dois alvos de ecrã eram **incapazes de exibir o defeito** — o kiosk já validava o uuid e a carta lê de um instantâneo em memória, logo os 404 deles eram das rotas e não da tradução. Trocados por `/platform/{id}` e `/platform/{id}/entitlements`, que davam 500. E o número fugiu QUATRO vezes — `P2007`, `P2010`, `22P02`, `P2023` — até a condição passar a perguntar pela FRASE do Postgres em vez de um número de uma lista; a extensão do lado do ecrã também só via `$allModels` e as rotas de `/platform` falam por `$queryRaw`. A MINHA PROVA, e é a que fecha: corri o mesmo plante `(null)` nos dois lados. Limpo dá verde, 4/4 nas duas vias. Na via de SESSÃO dá vermelho (`organization/unidades`, `puerto/orders` a 500). Na via de ECRÃ dá agora vermelho (`/platform/…`, `/platform/…/entitlements` a 500) — **o mesmo experimento que às 11h dava verde**. A guarda passou a exigir que, com a tradução inerte, CADA alvo deixe de dar 404: um alvo que continue em 404 não mede a tradução, mede a rota, e ela diz isso em vez de o aceitar como prova. Planta em todas as chamadas encontradas, e não numa lista escrita à mão. Antes:  A cura do JR é real: o código era o **P2007** e não o P2023, logo a primeira versão esteve INERTE em todos os caminhos, e a minha guarda dava 128/128 por cima de um mecanismo que nunca disparou. A nova guarda mede HTTP a sério — provei-o. MAS o controlo negativo só cobre metade: apliquei o MESMO plante (`ehIdentificadorMalFormado(null)`, que mantém todos os símbolos usados para o build sobreviver) aos dois lados. Na via de SESSÃO fica VERMELHA, duas rotas a 500 (`organization/unidades/…`, `puerto/orders/…`) — o build está vivo e a medição é real. Na via de ECRÃ fica VERDE, `ecra=2 falhas=0`, com a tradução desligada. **Aqueles dois 404 não vêm da tradução** — são o «não encontrado» próprio das rotas, exactamente a armadilha que o JR diagnosticou hoje e que a guarda nova ainda carrega em metade da população. PROVADO: a via de sessão. POR PROVAR: a via de ecrã. Reabri-o primeiro por isto:  A ponta que eu aceitei por declaração — «o 404 CHEGA ao cliente» — foi medida por outro agente e DISCORDA: o `M03 erro` continua a dar 500 com o directório de build isolado, logo não é colisão. E ao medir apareceu o buraco: **há TRÊS invólucros de escopo e a cura cobre dois**. `comEscopo` (escopo.ts:131) e `comIdentidade` (203) traduzem o `P2023`; **`comEscopoSerializavel` (159) NÃO TEM tradutor** — só `exigirUuid('organizationId')`, que guarda um campo e deixa cru qualquer outro id tocado dentro da transacção (usado em `reservas.ts:348` e `:588`). O MEU ERRO DE VERIFICAÇÃO, e é o que interessa: escrevi «a partição fecha — 128 páginas, 128 alcançam o invólucro». **Alcançar o invólucro não é devolver 404.** A partição era estanque sobre a propriedade errada: mediu ADOPÇÃO do mecanismo, não o seu RESULTADO. Uma guarda assim fica verde enquanto o mecanismo falha. Por medir ainda: se o 500 do M03 vem deste buraco ou de outro (o candidato do implementador — `instanceof` a falhar por `transpilePackages` duplicar a classe — está por confirmar e ele rotulou-o como hipótese)
```
