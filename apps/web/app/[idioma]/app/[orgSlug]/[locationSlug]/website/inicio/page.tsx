...
/**
 * WEB-003 · «Edita la portada» (atlas p. 247)\n *\n * O texto é texto simples e não HTML. Aceitar marcação de quem edita é\n * aceitar script de quem edita, e o E08 já mostrou o que custa confiar em\n * entrada — os parágrafos saem das linhas em branco, que é o que as pessoas\n * escrevem sem lhes ensinar nada.
 */
export default async function ConteudoDaPagina({
  ...
  const guardada = rascunho?.paginas.find((p) => p.tipo === 'INICIO');
  ...
  return (
    <div className="bo-pagina">
      ...
      <NavegacaoDoSite idioma={idioma} orgSlug={orgSlug} locationSlug={locationSlug} actual="/inicio" />
      ...
      <form method="post" action={`/api/org/${orgSlug}/site/pagina`} className="bo-publico__formulario">
        ...
        <input type="hidden" name="seccao" value="/inicio" />
