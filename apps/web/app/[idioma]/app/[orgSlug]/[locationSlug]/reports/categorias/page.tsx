...
import { porCategoria } from '@bossaos/db';
...
export default async function RelatorioAgrupado({
  ...
  const linhas = janela
    ? await comEscopoDoPedido(sessao, (db) => porCategoria(db, unidade.id, janela.de, janela.ate))
    : [];
    ...
    <div className="bo-pagina">
      <div className="bo-estado__cabecalho">
        <div>
          ...
          <h1>{p.porCategoria}</h1>
        </div>
      </div>

      <NavegacaoDeRelatorios idioma={idioma} orgSlug={orgSlug} locationSlug={locationSlug} actual="/categorias" />

      {!janela ? (
        <Aviso tom="aviso" titulo={p.porCategoria}>{m.salaE13.semFuso}</Aviso>
      ) : (
        <TabelaDeRelatorio idioma={idioma} titulo={p.porCategoria} linhas={linhas} />
      )}
    </div>
  );
}
