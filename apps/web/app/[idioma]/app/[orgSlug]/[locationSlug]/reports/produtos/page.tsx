...
import { porProduto } from '@bossaos/db';
...
export default async function RelatorioAgrupado({
  ...
  const linhas = janela
    ? await comEscopoDoPedido(sessao, (db) => porProduto(db, unidade.id, janela.de, janela.ate))
    : [];
    ...
    <div className="bo-pagina">
      <div className="bo-estado__cabecalho">
        <div>
          ...
          <h1>{p.produtosServidos}</h1>
        </div>
      </div>

      <NavegacaoDeRelatorios idioma={idioma} orgSlug={orgSlug} locationSlug={locationSlug} actual="/produtos" />

      {!janela ? (
        <Aviso tom="aviso" titulo={p.produtosServidos}>{m.salaE13.semFuso}</Aviso>
      ) : (
        <TabelaDeRelatorio idioma={idioma} titulo={p.produtosServidos} linhas={linhas} />
      )}
    </div>
  );
}
