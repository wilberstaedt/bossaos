...
import { porMesa } from '@bossaos/db';
...
export default async function RelatorioAgrupado({
  ...
  const linhas = janela
    ? await comEscopoDoPedido(sessao, (db) => porMesa(db, unidade.id, janela.de, janela.ate))
    : [];
  ...
  return (
    <div className="bo-pagina">
      ...
      <NavegacaoDeRelatorios idioma={idioma} orgSlug={orgSlug} locationSlug={locationSlug} actual="/mesas" />

      {!janela ? (
        <Aviso tom="aviso" titulo={p.porMesa}>{m.salaE13.semFuso}</Aviso>
      ) : (
        <TabelaDeRelatorio idioma={idioma} titulo={p.porMesa} linhas={linhas} />
      )}
    </div>
  );
}
