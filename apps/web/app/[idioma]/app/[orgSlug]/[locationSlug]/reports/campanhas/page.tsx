...
/**
 * REP-015 · «Qué trae cada campaña» (atlas)
 *
 ...
 */
export default async function PorCampanha({
  params,
    ...
    <div className="bo-pagina">
      <div className="bo-estado__cabecalho">
        <div>
          ...
          <h1 data-tela="REP-015">{t.campanhas}</h1>
        </div>
      </div>
      <p data-teste="ausencia-nao-e-zero">{t.ausenciaNaoEZero}</p>
      <p data-teste="definicao">{t.definicao}: {t.envios}</p>
      ...
      {r.medido ? (
        <>
          ...
          <ul className="bo-lista bo-lista--colunas" data-teste="linhas">
            {r.valor.map((l, i) => (
              <li key={i}>
                <span data-teste="campanha">{l.campanha}</span>
                <span data-teste="envios">{l.envios}</span>
              </li>
            ))}
          </ul>
        </>
