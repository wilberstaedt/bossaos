...
export async function varrerDescidas(
  ...
  // a descer de plano.
  const organizacoes = await descidasDevidas(prisma);
  ...
  for (const organizationId of organizacoes) {
    try {
      const r = await comEscopo(prisma, { organizationId }, async (db): Promise<ResultadoDaDescida> => {
        const resultado = await aplicarDescidaAgendada(db, organizationId);
        // O registo vai DENTRO da transacção. Auditar fora dela deixaria uma
        // linha a dizer que se desceu de plano num dia em que a descida foi
        ...
        await registar(db, organizationId, {
          ...
          // processo, que é a resposta honesta a "quem fez isto" numa linha de
          // auditoria que ninguém pediu.
          actorEmail: 'worker',
